...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_MaglevModel_process_check_sum_call( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_MaglevModel_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_MaglevModel_autoinheritance_info( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_MaglevModel_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_MaglevModel_get_eml_resolved_functions_info( int nlhs,
  ...
  if (strcmp(machineName, "MaglevModel") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_MaglevModel_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_MaglevModel_method_dispatcher(SimStruct *S, unsigned int
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"MaglevModel")) {
    return(sf_MaglevModel_method_dispatcher(simstructPtr,chartFileNumber,
...
extern void MaglevModel_terminator(void);
...
extern void MaglevModel_initializer(void);
extern void MaglevModel_register_exported_symbols(SimStruct* S);
extern void MaglevModel_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  MaglevModel_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      MaglevModel_debug_initialize();
    }

    MaglevModel_initializer();
  }
