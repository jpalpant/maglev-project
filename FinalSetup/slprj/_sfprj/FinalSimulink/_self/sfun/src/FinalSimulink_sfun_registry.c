...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_FinalSimulink_process_check_sum_call( int nlhs, mxArray
    * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_FinalSimulink_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_FinalSimulink_autoinheritance_info( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_FinalSimulink_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_FinalSimulink_get_eml_resolved_functions_info( int nlhs,
  ...
  if (strcmp(machineName, "FinalSimulink") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_FinalSimulink_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_FinalSimulink_method_dispatcher(SimStruct *S, unsigned
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"FinalSimulink")) {
    return(sf_FinalSimulink_method_dispatcher(simstructPtr,chartFileNumber,
...
extern void FinalSimulink_terminator(void);
...
extern void FinalSimulink_initializer(void);
extern void FinalSimulink_register_exported_symbols(SimStruct* S);
extern void FinalSimulink_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  FinalSimulink_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      FinalSimulink_debug_initialize();
    }

    FinalSimulink_initializer();
  }
